...
// Calcula suma total y subescalas (si existen reglas en dbo.subescalas). Tambi茅n reconoce LSAS por sufijos |miedo y |evitacion en questionId.
...
const createResponse = async (req, res) => {
  try {
    ...
    if (Number.isNaN(sid) || Number.isNaN(uid) || !Array.isArray(answers)) {
      return res.status(400).json({ message: 'Payload inv谩lido.' });
    }
    ...
    // No usamos puntaje_total ni resumen_json en esta inserci贸n.
    ...
    try {
      ...
      // Clasificaci贸n (opcional) para algunas escalas sin promedios
      try {
...
const getResponses = async (_req, res) => {
  try {
    ...
    for (const row of rs.recordset) {
      ...
      // Clasificaci贸n b谩sica para LSAS y FLCAS
      try {
...
const getResultsByUser = async (req, res) => {
  try {
    ...
    if (Number.isNaN(userId)) return res.status(400).json({ message: 'ID de usuario inv醠ido' });
    ...
    for (const row of rs.recordset) {
      ...
      // Clasificaci髇 b醩ica LSAS / FLCAS (solo para mostrar)
      try {
        if (totals && totals.subscales && (typeof totals.subscales['Miedo/ansiedad'] !== 'undefined' || typeof totals.subscales['Evitaci髇'] !== 'undefined')) {
          const t = totals.total || 0;
          let cls = 'Leve ansiedad social';
          if (t >= 82) cls = 'Grave Ansiedad Social';
          else if (t >= 52) cls = 'Moderada Ansiedad social';
    ...
    res.status(500).json({ message: 'Error interno del servidor.' });
      ...
      // Clasificaci贸n b谩sica LSAS / FLCAS
      try {
        if (totals && totals.subscales && (typeof totals.subscales['Miedo/ansiedad'] !== 'undefined' || typeof totals.subscales['Evitaci贸n'] !== 'undefined')) {
          const t = totals.total || 0;
          let cls = 'Leve ansiedad social';
          if (t >= 82) cls = 'Grave Ansiedad Social';
          else if (t >= 52) cls = 'Moderada Ansiedad social';
